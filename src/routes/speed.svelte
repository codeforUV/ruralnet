<script>
  ...
  function speedtestUpdate(data) {
    ...
    document.getElementById('result').innerHTML = `Ping: ${testInfo.ping} ms, Down: ${
      testInfo.downloadSpeed || 'N/A'
    } Mbps, Up: ${testInfo.uploadSpeed || 'N/A'} Mbps`;
    document.getElementById('done').textContent = `${
      ['not started', 'started', 'download', 'ping and jitter', 'upload', 'finished', 'aborted'][
        data.testState + 1
      ]
    }`;
  }
  async function speedtestEnd(aborted) {
    ...
    if (!aborted) {
      ...
      if (testInfo.latitude && testInfo.longitude) {
        let cityreq = await fetch(
          `/location/city.json?latlng=${testInfo.latitude},${testInfo.longitude}`
        );
        let cityJson = await cityreq.json();
        testInfo.city = `${cityJson.city}, ${cityJson.state}`;
        document.getElementById(
          'result'
        ).innerHTML += `, <a href="https://google.com/maps/search/${testInfo.latitude},${testInfo.longitude}">Location: ${testInfo.city}</a>`;
      } else {
  ...
  async function fixLocation() {
    ...
    if (good) {
      ...
      if (verification.verified) {
        let update = {
          // update keys need to match schema
          _id: testId,
        ...
        let updateReq = await fetch('/speedDatabase.json', {
          // speedDatabase upgraded to handle updates through POST, given known ID
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        document.getElementById(
          'chastise'
        ).innerText = `verified ${update.city} @ ${update.latitude}, ${update.longitude} for test #${update._id}`;
      } else {
        // if not verified, then user gave garbage. advise to retry
        document.getElementById('chastise').innerText =
          'Not a real town or zip code, check spelling and try again';
      }
    } else {
      document.getElementById('chastise').innerText =
        'please separate town + state with a comma, or enter a valid 5-digit zip code';
    }
  }
